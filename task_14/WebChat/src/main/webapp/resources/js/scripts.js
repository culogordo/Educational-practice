...
function deleteResponseToBoolean (serverResponse) {
...
function getServerResponse (continueWith) {
	...
	get(url, function(responseText) {
		...
		if (+token.substr(2, token.length-4) < +response.token.substr(2, response.token.length-4)) {
			response.message = deleteResponseToBoolean(response.message);
			if (token === 'TN11EN')	{
...
function storeSend(_deleted, _message, _author, _date, _id, _methodRequest) {
	return {
		deleted: _deleted,
		message: _message,
 		author:  _author,
...
function send (event) {
	...
	if (delEmptyText !== '')  {
		...
		currentUserName.textContent, currentTime, uniqueId(), 'POST');
